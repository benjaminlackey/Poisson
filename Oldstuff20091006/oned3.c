/*To compile type: gcc -lm -lgsl -lgslcblas -Wall -pedantic -ansi oned3.c */
...
#define Ne 5 /*Number of terms in matrices for even functions.*/
...
/*   printmatrix(L_out, Nall, Nall); */
  ...
  /*printmatrix(T_out, Nall, Nall);*/
  
  /*Make operator T.L*/
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 
		 1.0, T_out, L_out,
		 0.0, TL_out);
  printmatrix(TL_out, Nall, Nall);
  
  /*Make source vector.*/
  for(i=0; i<Nall; i++){
    gsl_vector_set(s_out, i, source_out(-cos(i*PI/(Nall-1))));
    /*printf("%f\n", -cos(i*PI/(Nall-1)));*/
  }
  printvector(s_out, Nall);
  ...
  printvector(u_out, Nall);
  
  ucoeff[0] = gsl_vector_get(u_out, 0);
  for(i=1; i<Nall; i++){
    ucoeff_out[i] = gsl_vector_get(u_out, i);
  }
  
  for(x=-1.0; x<=end; x+=0.01)
    printf("%f\t%f\n", x, chebeval(start, end, &ucoeff_out[0], Nall-1, x));
