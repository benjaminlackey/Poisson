...
main (void)
{
  ...
  gsl_matrix *L_even = gsl_matrix_alloc (n/2, n/2); /*even part of L*/
  gsl_matrix *invD_even = gsl_matrix_alloc (n/2, n/2); /*even part of L*/
  gsl_matrix *T = gsl_matrix_alloc (n/2, n/2); /*T_ki = T_i(x_k)*/
  gsl_matrix *TL = gsl_matrix_alloc (n/2, n/2);
  gsl_vector *s = gsl_vector_alloc (n/2); /*source in T_n basis*/
  gsl_vector *u = gsl_vector_alloc (n/2); /*solution in T_n basis*/
  ...
  printmatrix(L_even, n/2, n/2);
  
  /*Make colocation matrix T_ki = T_i(x_k) where x_k = sin(k*PI/(2*ORDER)).*/
  for(i=0; i<=ORDER; i++)
    for(j=0; j<=ORDER; j++)
      gsl_matrix_set (T, i, j, neg1toi(j)*cos(i*j*PI/ORDER));
  printmatrix(T, n/2, n/2);
  
  /*Make operator T.L*/
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 
		 1.0, T, L_even,
		 0.0, TL);
  printmatrix(TL, n/2, n/2);
  
  /*Make source vector.*/
  for(i=0; i<=ORDER; i++){
    gsl_vector_set(s, i, source(sin(i*PI/(2*ORDER))));
    printf("%f\n", sin(i*PI/(2*ORDER)));
  }
  printvector(s, ORDER+1);
